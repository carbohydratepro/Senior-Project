...
def tokens_to_code(tokenizer, tokens):
    ...
    """
    return tokenizer.decode(tokens[0])  # Take the first element of the predictions
...
def main():
    ...
    # 計算
    dataset_num = int(data_num*0.7)

    # データセットの読み込み
    data = read_data(100)
    datasets = create_datasets(data)
    problems = [data[0] for data in datasets][0:dataset_num]
    programs = [data[1] for data in datasets][0:dataset_num]
    test_problems = [data[0] for data in datasets][dataset_num:data_num]
    test_programs = [data[1] for data in datasets][dataset_num:data_num]
    ...
    for problems, programs in test_dataloader:
        ...
        with torch.no_grad():
            ...
            # LSTMの出力を配列に格納
            print("LSTM output (token IDs):", predicted[0].tolist())  # Take the first element of the predictions
            print("LSTM output (code):", tokens_to_code(tokenizer, (predicted.tolist().clear(100))))
